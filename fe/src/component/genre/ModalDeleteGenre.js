	...
	const handleDeleteGenre = async (event) => {
		try {
			...
			if (res.Success === true) {
				...
				toast.error(res.Message || "Error when deleting genre!");
				onClose();
			}
		} catch (error) {
						...
						<div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
							<div className="sm:flex sm:items-start">
								<div className="mx-auto flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-100 sm:mx-0 sm:h-10 sm:w-10">
									<BsExclamationTriangle aria-hidden="true" className="h-6 w-6 text-red-600" />
								</div>
								<div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
									...
									<div className="mt-2">
										<p className="text-sm text-gray-500">
											Are you sure you want to delete this genre? The data will be permanently removed. This action
											cannot be undone.
										</p>
									</div>
								</div>
